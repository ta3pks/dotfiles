...
```bash
INIT=$(node /home/nikos/.config/opencode/get-shit-done/bin/gsd-tools.cjs init phase-op "${after_phase}")
```
...
```bash
RESULT=$(node /home/nikos/.config/opencode/get-shit-done/bin/gsd-tools.cjs phase insert "${after_phase}" "${description}")
```
